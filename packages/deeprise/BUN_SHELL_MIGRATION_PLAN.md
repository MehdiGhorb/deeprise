...
Replace runtime Bun shell template-tag usage in `packages/deeprise/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/deeprise/src` is removed except:
