...
Practical reference for extracting a future `packages/server` from the current `packages/deeprise` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all deeprise services, Effect-first source of truth
- `packages/server` - deeprise server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full deeprise server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/deeprise`.
...
The intended direction here is the same, but the current `deeprise` package split is earlier in the migration.
...
- the current server host still lives in `packages/deeprise`
...
- if `packages/server` imports services from `packages/deeprise` while `packages/deeprise` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/deeprise` as the temporary Hono host
...
- `packages/server` must not import from `packages/deeprise`

Allowed in phase 1:

- `packages/deeprise` imports `packages/server`
...
- `packages/deeprise` shrinks or disappears as package responsibilities are fully split
...
- `packages/deeprise` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/deeprise`
...
Keep in `packages/deeprise` for now:
...
- host mounting from `packages/deeprise`
...
- no imports from `packages/deeprise` inside `packages/server`
...
- `packages/server` must still not import from `packages/deeprise`
...
### PR 4. Mount `packages/server` question from `packages/deeprise`

Scope:

- replace local experimental question route wiring in `packages/deeprise`
...
- mount it in parallel from `packages/deeprise`
...
- move adapter selection and server startup out of `packages/deeprise`

Rules:

- do not start this while `packages/server` still depends on `packages/deeprise`
...
- host logic no longer lives in `packages/deeprise`
...
3. the route is mounted from the current host in `packages/deeprise`
...
- `packages/server` imports services or runtime from `packages/deeprise`
- `packages/deeprise` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/deeprise` imports
...
- [ ] mount `question` from `packages/deeprise`
...
2. keep `packages/deeprise` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/deeprise`, stop and restructure the boundary first.
