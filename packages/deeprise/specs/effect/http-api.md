...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-deeprise-directory`.
...
8. Run `bun typecheck` from `packages/deeprise`, relevant `bun run test:ci ...` tests from `packages/deeprise`, and `./packages/sdk/js/script/build.ts` from the repo root.
...
- Cover `directory` and `x-deeprise-directory` instance selection.
