...
- Author package entrypoint is `@deeprise-ai/plugin/tui`.
...
```json
{
  "$schema": "https://deeprise.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/deeprise-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@deeprise-ai/plugin/tui`.
- `@deeprise-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@deeprise-ai/plugin/tui"
...
- If `engines.deeprise` is absent, no check is performed (backward compatible).
...
- `deeprise plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `deeprise plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.deeprise` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.deeprise`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.deeprise`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.deeprise` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@deeprise-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@deeprise-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.deeprise/themes` area near the plugin config source.
...
- Local smoke plugin: `.deeprise/plugins/tui-smoke.tsx`
- Local vim plugin: `.deeprise/plugins/tui-vim.tsx`
- Local smoke config: `.deeprise/tui.json`
- Local smoke theme: `.deeprise/plugins/smoke-theme.json`
