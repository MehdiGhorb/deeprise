...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run deeprise",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await Process.text(["deeprise", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting deeprise...")
        UI.println()

        const deepriseArgs = sessionId ? ["-s", sessionId] : []
        const deepriseProcess = Process.spawn(["deeprise", ...deepriseArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await deepriseProcess.exited
        if (code !== 0) throw new Error(`deeprise exited with code ${code}`)
      },
    })
  },
})
