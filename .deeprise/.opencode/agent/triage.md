...
- `deeprise web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `deeprise web` view. **Do not** add for terminal, TUI, or general deeprise issues.
...
**Only** add if the issue mentions "zen" or "deeprise zen" or "deeprise black".

If the issue doesn't have "zen" or "deeprise black" in it then don't add zen label

#### core

Use for core server issues in `packages/deeprise/`, excluding `packages/deeprise/src/cli/cmd/tui/`.
...
TUI (`packages/deeprise/src/cli/cmd/tui/...`):
...
Core (`packages/deeprise/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for deeprise server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
